...
<TechBadge text="Rust" bg="bg-orange-900 border-orange-700" />
<TechBadge text="Redis" bg="bg-red-900 border-red-700" />
<TechBadge text="System Design" bg="bg-blue-900 border-blue-700" />
...
<CodeGroup>
  <CodeGroupItem title="Unbounded Spawn (Danger)">
```javascript
...
```
  ...
  <CodeGroupItem title="Pooled Execution (Safe)" isOptimized={true}>
```javascript
...
<Callout type="danger" title="Timeout">
  ...
  <br />
  Spawning 100 math requests at once statically creates 800 independent
  background OS-level threads. The memory footprint skyrockets, and OS-context
  switching completely consumes the CPU time.
</Callout>
...
<Callout type="info" title="Decoupled Architecture Flow">
  <ol>
    <li>
      The <strong>API Gateway</strong> intercepts the HTTP request.
    </li>
    <li>
      It pushes the payload data directly into a dedicated Message Broker
      (Redis/BullMQ).
    </li>
    <li>
      Node.js replies instantly: <em>"HTTP 202 Accepted"</em>.
    </li>
    <li>
      A completely separate backend application natively polls the Redis stack,
      executes the heavy work, and updates the database.
    </li>
  </ol>
</Callout>
...
<KeyTakeaway>
  ...
  module to a <b>Rust Microservice</b> yields a blistering 5x latency advantage.
  ...
  <b>Enterprise Messaging Queue</b>.
</KeyTakeaway>
